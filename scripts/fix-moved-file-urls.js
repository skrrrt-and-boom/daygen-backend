...
console.log('🔧 Fixing Moved File URLs...\n');
console.log('Target Public URL:', R2_PUBLIC_URL);

async function fixMovedFileUrls() {
  try {
    // Find all images with old R2 URLs that need path correction
    ...
    for (const image of oldR2Images) {
      try {
        ...
        // Create new URL with the correct public endpoint and generated-images/ path
    ...
    // Show summary of all R2 images
    console.log(`\n📊 Final R2 Image Summary:`);
    const allR2Images = await prisma.r2File.findMany({
      where: {
        deletedAt: null,
        fileUrl: {
    ...
    console.log(`✅ Active R2 images: ${allR2Images.length}`);
    allR2Images.forEach(img => {
      console.log(`   - ${img.fileName}: ${img.fileUrl}`);
    });
