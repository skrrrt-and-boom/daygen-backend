...
export class GenerationService {
  ...
  private async dispatch(model: string, dto: UnifiedGenerateDto): Promise<ProviderResult> {
    switch (model) {
  ...
  private async handleGemini(dto: UnifiedGenerateDto): Promise<ProviderResult> {
    ...
    const primaryInline = this.normalizeInlineImage(dto.imageBase64, dto.mimeType || 'image/png');
    if (primaryInline) {
    ...
    if (!imgPart?.inlineData?.data) {
      throw new BadRequestException('No image returned from Gemini 2.5 Flash Image');
    }
  ...
  private async handleIdeogram(dto: UnifiedGenerateDto): Promise<ProviderResult> {
  ...
  private async handleQwen(dto: UnifiedGenerateDto): Promise<ProviderResult> {
    ...
    const body = {
      ...
      parameters: {
        ...
        ...(typeof dto.providerOptions.seed === 'number' ? { seed: dto.providerOptions.seed } : {}),
    ...
    if (!response.ok) {
      this.logger.error(`DashScope error ${response.status}: ${JSON.stringify(result)}`);
      throw new HttpException(
  ...
  private async handleRunway(dto: UnifiedGenerateDto): Promise<ProviderResult> {
    ...
    const model = dto.model === 'runway-gen4-turbo' ? 'gen4_image_turbo' : 'gen4_image';
  ...
  private async handleSeedream(dto: UnifiedGenerateDto): Promise<ProviderResult> {
    ...
    const result = await response.json();
    if (!response.ok) {
      this.logger.error(`Seedream API error ${response.status}: ${JSON.stringify(result)}`);
      throw new HttpException(
  ...
  private async handleChatGpt(dto: UnifiedGenerateDto): Promise<ProviderResult> {
    ...
    const result = await response.json();
    if (!response.ok) {
      this.logger.error(`OpenAI API error ${response.status}: ${JSON.stringify(result)}`);
      throw new HttpException(
  ...
  private async handleReve(dto: UnifiedGenerateDto): Promise<ProviderResult> {
    ...
    const result = await response.json();
    if (!response.ok) {
      this.logger.error(`Reve API error ${response.status}: ${JSON.stringify(result)}`);
      throw new HttpException(
  ...
  private async handleRecraft(dto: UnifiedGenerateDto): Promise<ProviderResult> {
    ...
    const result = await response.json();
    if (!response.ok) {
      this.logger.error(`Recraft API error ${response.status}: ${JSON.stringify(result)}`);
      throw new HttpException(
  ...
  private normalizeInlineImage(value: unknown, fallbackMime: string): InlineImage | null {
  ...
  private async downloadAsDataUrl(url: string, headers?: Record<string, string>) {
